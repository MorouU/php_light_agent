...
# 遇到跳转时的请求方法
$REQUEST_REDIRECT_METHOD = $_REQUEST['rmethod'];
...
# COOKIE数据
$REQUEST_COOKIE= $_REQUEST['cookie'];

# 其他头数据，目前支持socket,file_get_contents
...
class Agent{
    ...
    private function get302_socket($url)
    {
        try {
            ...
            if (!$fp) {
				...
				if(is_array($this->request_headers) and count($this->request_headers) > 0){
    ...
    public function __construct($request_url,$request_method,$request_redirect_method,$redirect_auto,$redirect_cookie_use,$params,$data,$cookie,$headers,$timeout,$type){
...
$this_request = new Agent($REQUEST_URL,$REQUEST_METHOD,$REQUEST_REDIRECT_METHOD,$REDIRECT_AUTO,$REDIRECT_COOKIE_USE,$REQUEST_PARAMS,$REQUEST_DATA,$REQUEST_COOKIE,$REQUEST_HEADERS,$TIMEOUT,$TYPE);
echo $this_request;
